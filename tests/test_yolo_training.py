...
from bookworm.yolo_training import DEFAULT_BASE_WEIGHTS, DEFAULT_EPOCHS, DEFAULT_IMAGE_SIZE, run_yolo_fine_tune
...
def test_run_yolo_fine_tune_passes_explicit_options_through(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    best_weights_path = run_yolo_fine_tune(dataset_yaml, base_weights=base_weights, epochs=10, image_size=320)

    assert constructor_calls == [base_weights]
    assert train_calls == [{"data": str(dataset_yaml), "epochs": 10, "imgsz": 320}]
...
def test_run_yolo_fine_tune_uses_defaults_when_not_given(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert train_calls == [{"data": str(dataset_yaml), "epochs": DEFAULT_EPOCHS, "imgsz": DEFAULT_IMAGE_SIZE}]
