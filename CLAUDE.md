...
- T16 (2026-09-16): `gdown.download_folder` names local files after the Drive filename. Two distinct files sharing a name (e.g. two different `IMG_0034.HEIC` uploads) collide on one local path; `resume=True` sees the path already exists and skips the second file entirely, silently dropping it. Caught by running `fetch-drive` against a real folder, not by unit tests with mocked, uniquely-named fakes. Fix: list first (`skip_download=True`), then download each file individually, prefixing any name that isn't unique with its Drive file ID.
- T17 (2026-09-16): `bookworm annotator` defaults to port 8765. If a real annotator session is already running there, a second `bookworm annotator ... --port 8765` fails to bind, but a plain health-check `curl` on that port still returns 200 — against the *first* process, not yours. Browser-testing the annotator against a synthetic fixture must pass an explicit `--port` and confirm the PID/photo list match your fixture before interacting, not just check for a 200.
- T18 (2026-09-16): Dispatching a synthetic `new PointerEvent(...)` via `element.dispatchEvent(...)` in chrome-devtools MCP works for `pointerdown`/`pointermove`/`pointerup`, but `element.setPointerCapture(pointerId)` throws `NotFoundError: No active pointer with the given id is found` for any `pointerId` other than `1` (the browser's real mouse pointer), since the fake event never registers as an active platform pointer. The throw aborts the rest of the handler silently — state mutations after that line never run, with no visible symptom beyond "nothing happened." Always use `pointerId: 1` for synthetic pointer-event drag simulations against code that calls `setPointerCapture`.
