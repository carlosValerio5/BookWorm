...
- T16 (2026-09-16): `bookworm annotator` defaults to port 8765. If a real annotator session is already running there, a second `bookworm annotator ... --port 8765` fails to bind, but a plain health-check `curl` on that port still returns 200 — against the *first* process, not yours. Browser-testing the annotator against a synthetic fixture must pass an explicit `--port` and confirm the PID/photo list match your fixture before interacting, not just check for a 200.
