...
- `bookworm dashboard` opens a local web app at `http://127.0.0.1:8766` showing the latest `train-yolo` run's loss (train vs val, updated live from `results.csv`) and validation accuracy (precision, recall, mAP50, mAP50-95 — YOLO detection has no train-side accuracy metric, so this panel is validation-only). Styled after the annotator's dark window aesthetic (F2).
